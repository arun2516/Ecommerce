...
export default function Category() {
    ...
    const createCategory = async() => {
        try {
            if(edit) {
                const response = await axios.put(`http://shoppeecom.herokuapp.com/api/category/${id}`, {name: category}, {
                    headers: {Authorization: token}
                })
                Swal.fire("Success!", response.data.msg, "success")
            }
            else {
                const response = await axios.post(`http://shoppeecom.herokuapp.com/api/category`, {name: category}, {
                    headers: {Authorization: token}
                })
                Swal.fire("Success!", response.data.msg, "success")
            }
    ...
    const deleteCategory = async(id) => {
         try {
             const response = await axios.delete(`http://shoppeecom.herokuapp.com/api/category/${id}`, {
                 headers: {Authorization: token}
             })
